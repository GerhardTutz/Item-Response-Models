...
################ marginal fits various models program margfit needed
...
if(respffit=="NV"){respfctd <- respfctdNV
respfctc<-respfctcNV
respfctder<-respfctderNV}
if(respffit=="logistic"){respfctd <- respfctdlogit
respfctc<-respfctclogit
respfctder<-respfctderlogit}
if(respffit=="Gumbel"){respfctd <- respfctdGumbel
respfctc<-respfctcGumbel
respfctder<-respfctderGumbel}
if(respffit=="Gompertz"){respfctd <- respfctdGompertz
...
P<- dim(datfull)[1]
I<- dim(datfull)[2]
datfullbin<-matrix(0,P,I)
for (p in 1:P){for (i in 1:I) {if (datfull[p,i] >=5) datfullbin[p,i]<-1}}



mfitfull<- MargFit(datfullbin,I=5)
mfitfull
