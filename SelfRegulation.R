...
#### Thresholds models for self-regulation data
...
#### choose difficulty function (lin), response function (respf), 
#and distribution continuous/discrete (indicator)
...
#(logit in (2) is special, comes later)
...
########################################
### only if(lin =="logit") compute first min, max, then define difficulty function
...
ThrGd<-ThreshModFixedResp(dat,commonslope="var" ,indicator="D", der="der",lin =lin, start, startind="no",respf)
ThrGd
...
plot(y,prob,cex.axis=1,cex.lab=.8,cex.main=1.0, main =titledens,xlab ="y", type="b",
     lwd=.4,ylim=ylims, ylab="", pch =pcdum[1],cex=1,pin=c(6,6))

for (i in 2:I){
   for (l in 1:length(y))prob[l]<- {respfctd(alphas[i]*theta - diffunct(parmatrest[i,1],parmatrest[i,2],y[l]))*
      derdiffunct(parmatrest[i,1],parmatrest[i,2],y[l])} 
  lines(y,prob,lty=2,lwd=.8, type="b",pch =pcdum[i],cex = 1)
  }
